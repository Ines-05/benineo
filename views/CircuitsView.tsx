    ...
    <div className="pt-24 pb-24 dark:bg-stone-950 transition-colors duration-500">
      ...
      <section className="bg-stone-50 dark:bg-stone-900 py-20 px-6 mb-20 transition-colors duration-500">
        <div className="max-w-7xl mx-auto">
          <span className="text-amber-700 dark:text-amber-500 font-bold uppercase tracking-[0.3em] text-[10px] mb-4 block">Découverte</span>
          <h1 className="text-5xl md:text-7xl font-serif text-stone-900 dark:text-white mb-8 max-w-4xl">Nos Circuits <span className="italic">Signature</span></h1>
          <p className="text-stone-600 dark:text-stone-300 text-lg max-w-2xl leading-relaxed font-light italic">
            "Béninéo, là où commence le monde, là où se crée votre histoire." Chaque itinéraire est un poème écrit sur la terre-mère.
          </p>
        </div>
      </section>
          ...
          <div key={index} className={`max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${index % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <h2 className="text-4xl md:text-5xl font-serif text-stone-900 dark:text-white leading-tight">{circuit.title}</h2>
                <p className="text-amber-700 dark:text-amber-500 italic font-medium tracking-wide">{circuit.tagline}</p>
              </div>

              <p className="text-stone-600 dark:text-stone-300 leading-relaxed text-lg italic border-l-4 border-amber-200 dark:border-amber-700 pl-8 py-2">
                "{circuit.description}"
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="p-8 bg-white dark:bg-stone-900 rounded-3xl border border-stone-100 dark:border-stone-800 shadow-sm transition-colors duration-500">
                  <div className="flex items-center gap-3 text-amber-700 dark:text-amber-500 mb-4">
                    <Clock size={20} />
                    <span className="font-bold uppercase tracking-widest text-[10px]">Circuit Classique</span>
                  </div>
                  <p className="text-stone-900 dark:text-white font-serif text-2xl mb-1">{circuit.duration}</p>
                  <p className="text-amber-900 dark:text-amber-400 font-bold text-lg">Dès {circuit.price}</p>
                </div>
                <div className="p-8 bg-white dark:bg-stone-900 rounded-3xl border border-stone-100 dark:border-stone-800 shadow-sm transition-colors duration-500">
                  <div className="flex items-center gap-3 text-amber-700 dark:text-amber-500 mb-4">
                    <Users size={20} />
                  ...
                  <p className="text-stone-900 dark:text-white font-serif text-2xl mb-1">{circuit.shortDuration}</p>
                  <p className="text-amber-900 dark:text-amber-400 font-bold text-lg">Dès {circuit.shortPrice}</p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                  {circuit.details.map((detail, i) => (
                    <div key={i} className="flex items-center gap-3 text-stone-700 dark:text-stone-300 text-sm">
                      <CheckCircle2 size={16} className="text-amber-600 dark:text-amber-500 shrink-0" />
              ...
              <div className="pt-8">
                <a
                  ...
                  className="bg-stone-950 dark:bg-stone-100 text-white dark:text-stone-900 px-12 py-5 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-amber-700 dark:hover:bg-amber-500 dark:hover:text-white transition-all inline-flex items-center gap-6 shadow-xl"
                >
